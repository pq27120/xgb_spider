...
class HtmlParser(object):
    ...
    def parse_article(self, html):
        ...
        try:
            article_title = soup.find('h2', class_='rich_media_title').get_text().strip(' \n').encode('utf-8')
            wname = soup.find('a', id='post-user').get_text().encode('utf-8')
            date = soup.find('em', id='post-date').get_text().encode('utf-8')
            content = soup.find('div', class_='rich_media_content ').encode('utf-8')#文章内容
        except Exception:
            return None
        return article_title, wname, date, content
